...
**Direction note (2026-09-13).** The table above assumes the case owns the
outline. Right now it does not: the main PCB outlines were generated by
kbplacer as a convex hull, so they currently flow **KiCad → CAD**, exported by
`hardware/export_outlines.py`. That is fine as a starting point, but ownership
should flip at **G3** — once the case has an opinion about the outline, the DXF
must come *from* CAD, and the generator's hull stops being authoritative.
Whichever direction is live, exactly one side owns it.
